...
;(() => {
  ...
  const getCssVar = name => {
    try {
      return getComputedStyle(MODE.cssRoot).getPropertyValue(name).trim()
    } catch {
      return ''
    }
  }
  ...
  const oklabToRgb = (L, a, b) => {
    ...
    const b3 = linearToSrgb(-0.0041960863 * l - 0.7034186147 * m + 1.707614701 * s)
    return { r, g, b: b3 }
  }
  ...
  const oklchToHexSafe = (L, C, h) => {
    let c = C
    for (let i = 0; i < 12; i++) {
      const { a, b } = oklchToOklab(L, c, h)
      const rgb = oklabToRgb(L, a, b)
      if (isInGamut(rgb)) return toHex(rgb)
      c = Math.max(0, c - 0.02)
    }
    return toHex(oklabToRgb(L, 0, 0))
  }
  const parseCssColorToRgb = css => {
    try {
  ...
  const generators = {
    ...
    diverging: (baseOKLCH, count) => {
      ...
      if (total % 2 === 1) {
        ...
        if (out.length) {
          out[0] = hexFromOKLab(leftLab.L, leftLab.a, leftLab.b)
          out[out.length - 1] = hexFromOKLab(rightLab.L, rightLab.a, rightLab.b)
        }
      } else {
        const nSide = total >> 1
  ...
  const updatePalettes = () => {
    ...
    try {
      document.dispatchEvent(new CustomEvent('palettes:updated', { detail: { primary: primaryHex, primaryOKLCH } }))
    } catch {}
  }
  ...
  const bootstrap = () => {
    ...
    const pickTextStyleForBackground = (bgCss, opts = {}) => {
      ...
      const getCssVar = name => {
        try {
          return getComputedStyle(cssRoot).getPropertyValue(name).trim()
        } catch {
          return ''
        }
      }
      ...
      try {
        ...
        const candidates = candidatesCss.map(css => ({ css, rgb: resolveCssToRgb01(css) })).filter(x => !!x.rgb)
        // Pick the max contrast
        let best = candidates[0]
        let bestCR = contrast(best.rgb, bg)
        ...
        const stroke =
          best.css === '#000' || best.css.toLowerCase() === 'black'
            ? `rgba(255,255,255,${0.3 + 0.4 * haloStrength})`
            : `rgba(0,0,0,${0.3 + 0.3 * haloStrength})`
